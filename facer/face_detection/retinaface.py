...
def batch_detect(net: nn.Module, images: torch.Tensor, threshold: float = 0.5):
    ...
    return {
        'rects': torch.stack(rects, dim=0).to(img.device) if len(rects) else [],
        'points': torch.stack(points, dim=0).to(img.device)if len(rects) else [],
        'scores': torch.tensor(scores).to(img.device)if len(rects) else [],
        'image_ids': torch.tensor(image_ids).to(img.device)if len(rects) else [],
    }
